...
var _health_icon: Texture = preload("res://gui/hud_health.tres")
...
func _refresh_health(vitals: VitalsComponent) -> void:
	var extra_health = vitals.current_health - MAX_ICONS
	if extra_health > 0:
	...
	for i in range(MAX_ICONS):
		...
		icon.visible = i < vitals.current_health


func _on_vitals_health_changed(vitals) -> void:
	_refresh_health(vitals)
